...
export default function TransferForm({updateSaldo}: {updateSaldo: (nominal: number) => void}) {
    ...
    async function handleTransfer() {
        if (noHP.length < 12 || noHP.length > 13 || Number.isNaN(nominal) || nominal < 1 || nominal > 1000000000 || !Number.isInteger(nominal)) {
            setFailedMessage(
                noHP.length < 12 || noHP.length > 13 ? 'Nomor HP tidak valid' :
        ...
        const response = await fetch('/api/tr-mypay/transfer', {
            method: 'POST',
            credentials: 'include',
            headers: {
            ...
            <div className="flex flex-col md:my-2 my-6">
                ...
                <div className="flex md:justify-end mt-10 md:mt-8" >
                    <button
                        ...
                        onClick={(e) => { e.preventDefault(); if(!loading) handleTransfer(); }}
                    >
                        {loading ? <CircularLoading black={true} size={`6`} /> : 'Transfer'}
